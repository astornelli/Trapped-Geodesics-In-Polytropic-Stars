# Neutral Massive Animation Code for 1/E^2
import numpy as np
import matplotlib
...
def update(frame):
    ...
    fmE = f"{one_over_esq:.1e}"
    digit, exponent = fmE.split('e')
    fmrho = f"{rho0:.1e}"
    digit1, exponent1 = fmrho.split('e')
    ax.set_title(rf"$1/\mathcal{{E}}^2  = {float(digit):.2f} \times 10^{{{int(exponent)}}}$", y=1.15)
    plt.suptitle(
