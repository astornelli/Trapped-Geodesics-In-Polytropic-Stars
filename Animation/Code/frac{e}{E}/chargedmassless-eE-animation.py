# Charged Massless Animation Code for e/E
import numpy as np
import matplotlib
...
def update(frame):
        ...
        for l in lvals:
            for g in gvals:
                ...
                if len(np.where(phip[1]<=0)[0])!=0:
                    continue
                F = cumulative_trapezoid(Fprime(n, l, g)*Rb, X, initial=0)
                Chargemassless = (1 - X*Rb*phip[0])/(X*Rb*(phip[0]*F-Fprime(n, l, g))-F)
    ...
    plt.suptitle(
        rf"$(\rho_0 = {float(digit1):.2f} \times 10^{{{int(exponent1)}}}, p_0/\rho_0 = {p0 / rho0:.2f}, Q = 10^3, r_b = 10^4)$",
        fontsize=12,
        y=0.9,
    )
...
ani.save("eE-ChargedMassless.mp4", writer=writer)
