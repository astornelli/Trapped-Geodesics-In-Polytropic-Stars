# Charged Massless Animation Code for Rb
import numpy as np
import matplotlib
...
def update(frame):
        ...
        for l in lvals:
            for g in gvals:
                ...
                if len(np.where(phip[1]<=0)[0])!=0:
                    continue
                F = cumulative_trapezoid(Fprime(n, l, g, Q)*Rb, X, initial=0)
                Chargemassless = (1 - X*Rb*phip[0])/(X*Rb*(phip[0]*F-Fprime(n, l, g, Q))-F)
    ...
    plt.suptitle(
        rf"$(\rho_0 = {float(digit1):.2f} \times 10^{{{int(exponent1)}}}, p_0/\rho_0 = {p0 / rho0:.2f}, r_b = 10^4, e/\mathcal{{E}} = 10)$",
        fontsize=12,
        y=0.9,
    )
...
ani.save("QChargedMassless.mp4", writer=writer)
