# Charged Massive Animation Code for E
import numpy as np
import matplotlib
...
def update(frame):
    ...
    ax.set_title(rf"$\mathcal{{E}} = {E:.1f}$", y=1.15)
    plt.suptitle(
        rf"$(\rho_0 = {float(digit):.2f} \times 10^{{{int(exponent)}}}, p_0/\rho_0 = {p0 / rho0:.2f}, Q = 10^3, e/\mathcal{{E}} = 100)$",
        fontsize=12,
        y=0.9,
    )
...
ani.save("E-ChargedMassive.mp4", writer=writer)
